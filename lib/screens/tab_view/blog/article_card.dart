...
import 'package:touriso/screens/tab_view/blog/article_details_page.dart';
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    DateFormat.yMMMEd()
                        .format(article.lastEdited ?? article.dateTimePosted),
            ...
            Text(article.content, maxLines: 5, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 12),
            if (article.siteIds.isNotEmpty && article.activityIds.isNotEmpty)
              InkWell(
                onTap: () {
                  showFormDialog(
                    context,
                    SingleChildScrollView(
                      child: Column(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          if (article.siteIds.isNotEmpty) const Text('Sites'),
            ...
            if (article.imageUrls.isNotEmpty) const SizedBox(height: 12),
            const SizedBox(height: 12),
            if (article.imageUrls.isNotEmpty) const SizedBox(height: 12),
            ClipRRect(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        ...
                        color: article.likes.contains(uid) ? Colors.red : null,
                      ),
                                ...
                                try {
                                  articlesCollection.doc(article.id).update({
                                    'comments': FieldValue.arrayUnion([
                                      Comment(userId: uid, content: comment)
                                          .toMap()
                                    ])
                                  });
